#!/usr/bin/env python3
"""
DISABLED 2026-09-04, PENDING CARSON'S DECISION. Nothing below this stub
runs — the whole 1,576-line module is commented out underneath it.

WHY IT IS DISABLED RATHER THAN DELETED
    This module renders the single-clip page you get at
    /<slug>/base/viewer.html and /<slug>/overlay/viewer.html — "open this
    scene on its own" from a pair.

    Three facts came out of the 2026-09-04 cleanup:

      1. NOTHING IN THE UI LINKS TO IT. There is no button, no menu item
         and no link anywhere in segment_avatar_editor/web/ pointing at
         those URLs. The only way to reach the page is to type it.
      2. It is no longer a duplicate of anything. It was 99% identical to
         mp4_splitter/player.py; that file became 65 lines on 2026-09-04
         when its page moved to static files. This one did not, so it is
         now the last Python-string page in the repo.
      3. write() runs on every extraction, so opening one pair writes TWO
         of these pages into the cache.

    So the question is whether the page is used at all. Disabling is how
    that gets answered: work normally for a while and see whether anything
    is missed. If nothing is, this file gets deleted. If something is, the
    commented body is still right here to bring back, and it migrates to
    static files like the other four pages did.

WHAT STILL WORKS WHILE IT IS OFF
    Everything except that one page. write() below is a live no-op, so
    editor_base.frames.write_viewer() still has something to call and no
    extraction breaks. Pairs, timelines, frame edits, Save, Cut, Join and
    Split are all untouched.

    ⚠ PAGES ALREADY ON DISK ARE STILL SERVED. This stops new ones being
    written; it does not remove the ones written before today. A cache
    folder that already has base/viewer.html will keep handing it out, so
    a clip opened before 2026-09-04 will still show the page. Clear them
    to test properly:

        find cache_segment_avatar_editor -path '*/base/viewer.html' -delete
        find cache_segment_avatar_editor -path '*/overlay/viewer.html' -delete

TO BRING IT BACK
    Strip the leading "# " from every line below and delete this stub.
"""

# Read by tests/test_segment_avatar_editor.py, which flips two of its
# checks while this is True: instead of asserting the page is served, they
# assert it is NOT — so the suite still proves what is actually true now
# rather than being switched off and quietly proving nothing.
DISABLED = True


def write(outdir, meta):
    """A live no-op. editor_base.frames.write_viewer() calls this after
    every extraction, so it has to exist and has to not raise."""
    return None


# ===========================================================================
# EVERYTHING BELOW IS THE ORIGINAL MODULE, COMMENTED OUT IN FULL.
# 1,576 lines. Do not edit it here — either restore it or delete the file.
# ===========================================================================

# #!/usr/bin/env python3
# """
# PRIVATE duplicate of mp4_splitter/player.py, copied here on 2026-09-02 so
# the Segment and Avatar Editor's "open this scene on its own" link keeps
# working without importing the actual mp4_splitter package — the whole
# point of splitting the two tools apart was that neither depends on the
# other's code. Called from this package's own frames.py (write_viewer()),
# never directly. Keep it in step with mp4_splitter/player.py BY HAND if
# that ever changes; there is no other link between the two any more.
#
