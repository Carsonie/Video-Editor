...
# This tool's OWN cache — not shared/'s cache/, and not MP4 Splitter's own
# separate copy either (see that package's own frames.py). Split apart on
# 2026-09-02, Carson's own call: fully separate cache folders, not just
# separate code, so the two tools' extracted-frame data never mixes even
# by accident.
HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)
CACHE = os.path.join(ROOT, "cache_segment_avatar_editor")
...
def write_viewer(outdir, meta):
    """
    (Re)write a clip's OWN page — from meta.json.
    ...
    Imports this PACKAGE's own private duplicate of the MP4 Splitter's
    player (_splitter_player.py) — not the real mp4_splitter package. The
    two tools stopped sharing code on 2026-09-02; this link is kept working
    on purpose (Carson's call), by duplication rather than a cross-package
    import. Imported late, inside the call, same reason as before: this
    frames.py is the layer the player is built ON, so importing it at
    module level would make frames depend on the thing that depends on it.
    """
    from segment_avatar_editor import _splitter_player as player
