...
# CODE_ROOT is the folder the editors live in — one level up from tests/.
CODE_ROOT = os.path.abspath(os.path.join(HERE, ".."))


def _find_customers(start):
    """
    Walk UP until a folder holds Customers/, the same way shared/serve.py's
    find_repo_root() does. Deliberately not a level count.

    It used to be `HERE/..` — five levels in Basic_E2E_Testing, then one here,
    and the comment on it said a count is wrong the moment the tree changes.
    That happened on 2026-09-04: the code moved into Video-Editors/ so that
    Customers/ could sit beside it, and every one of the six suites died at
    once looking for Video-Editors/Customers/. Walking up cannot go stale.
    """
    d = start
    while True:
        ...
        if parent == d:
            sys.exit("fixture: no Customers/ folder above " + start)
        d = parent


REPO = _find_customers(HERE)
