...
class Handler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed.path in ("/api/open", "/api/open-seq", "/api/open-seq-go"):
    ...
    def send_viewer(self, slug):
        """A timeline's page — segment_avatar_editor/web/seq.html,
        chosen by view.json.
...
<script>
  ...
  async function openPair() {
    // THE LAYERED VIEW IS GONE (2026-09-04, Carson's call).
    setStatus('The layered view has been removed \u2014 open the scene on a timeline instead.');
  }
